...
function moveNextNode(
	...
	if (!currentNode.isLarge) {
		if (smallCavesVisited.includes(currentNode.name)) {
			// console.log(`Finished path ${fullPath} on visited small cave ${currentNode.name})`);
			return;
		} else {
	...
	for (let connectionIndex = 0; connectionIndex < currentNode.connections.length; connectionIndex++) {
		moveNextNode(currentNode.connections[connectionIndex], [ ...smallCavesVisited ], fullPath, pathLength);
	}
...
export function runCompete(): void {
	...
	initMap();
	// displayMap();
	moveNextNode(findNode('start')!, [], 'Init', 0);

	console.log(`[Day 12 First] Result = ${successfulRoutes.length}`);
}
