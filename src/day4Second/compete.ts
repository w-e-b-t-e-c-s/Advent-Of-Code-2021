...
export function runCompete(): void {
	...
	let lastWinnerBoard: Array<Tile> | undefined = undefined;
	...
	while (inputTurns.length > 1 && allBoards.length) {
		currentMove = +inputTurns.shift()!;
		let processedBoards: Array<Array<Tile>> = [];
		allBoards.map((board) => {
			if (isWinnerBoard(board, currentMove)) {
